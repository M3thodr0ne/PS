...
#sauvegarder la matrice de calculs des populations a la fin de la boucle des iterations
def savePop(l,Mpop):
    mon_fichier = open("U"+str(l),"w")
    for i in range(len(Mpop)):
        for j in range(len(Mpop[i])):
            mon_fichier.write(str(Mpop[i][j]))
            if j < len(Mpop[i]) -1 :
                mon_fichier.write(",")
        mon_fichier.write("\n")
    mon_fichier.close()
